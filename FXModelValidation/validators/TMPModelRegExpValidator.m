...
#import "TMPModelRegExpValidator.h"

@implementation TMPModelRegExpValidator {
...
-(void)setPattern:(id)pattern {
	if([pattern isKindOfClass:[NSString class]]) {
		...
		@throw [NSException exceptionWithName:@"TMPModelRegExpValidator" reason:@"Invalid type of 'pattern'. Only NSString and NSRegularExpression are supported." userInfo:nil];
	}
}

-(NSError *)validateValue:(id)value {
	if(_regExp == nil)
		@throw [NSException exceptionWithName:@"TMPModelRegExpValidator" reason:@"The 'pattern' property must be set." userInfo:nil];
